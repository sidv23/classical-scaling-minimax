...
begin
    ...
    plts = [
        begin
            ...
            for (m, s) in enumerate(Sigmas)
                p = plot(p, Ns, n -> 2.0 * sqrt(q/(q-2.0)) * s .* √(kappa / n), ls=:dash, label="", c=cls3[m])
            end
            p
        end
    ...
    p11 = plot(dm, plts..., layout=l_1(), size=(900, 1000), scale=:log10, ylim=(2e-3, 1e1))
end
...
begin
    ...
    plts = [
        begin
            ...
            for (m, s) in enumerate(Sigmas)
                p = plot(p, Ns, n -> 1.5 * s * sqrt(q/(q-2)) .* kappa .* √(log(n) / n), ls=:dash, label="", c=cls3[m])
            end
            p
        end
        for (i, q) in enumerate(Qs), (j, kappa) in enumerate(Ks)
    ...
    dm = dm_1()
    dm = title!(dm, "\n" * L"L${}_{2\!\!\to\!\!\infty}$")
    dm = plot!(dm, zeros(1), c=:black, ls=:dash, label=L"${\sigma\kappa} \cdot \sqrt{\log{n}/n}$")
    p12 = plot(dm, plts..., layout=l_1(), size=(900, 1000), scale=:log10, ylim=(2e-3, 1e1))
end
...
begin
    ...
    plts = [
        begin
            ...
            p = plot(p, Ns, n -> 2.0 * s .* Ks[3] * √(1 / n), ls=:dash, label="", c=:black)
            p
        end
    ...
    p21 = plot(dm, plts..., layout=l_2(), size=(300, 230) .* (length(Noises), length(Sigmas)), scale=:log10, ylim=(5e-4, 1e3))
end
...
begin
    ...
    plts = [
        begin
            ...
            p = plot(p, Ns, n -> 1.5 * s .* Ks[3] * √(log(n) / n), ls=:dash, label="", c=:black)
            p
        end
        for (i, s) in enumerate(Sigmas), (j, noise) in enumerate(Noises)
    ]
    ...
    p22 = plot(dm, plts..., layout=l_2(), size=(300, 230) .* (length(Noises), length(Sigmas)), scale=:log10, ylim=(5e-4, 1e3))
end
